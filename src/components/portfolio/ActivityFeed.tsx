      ...
      {grouped.map(([label, items]) => (
        <div key={label}>
          <div className="mb-2 flex items-center gap-2">
            <p className="text-xs font-black uppercase text-[#8FE06A]">{label}</p>
            <div className="h-px flex-1 bg-[#8FE06A]/45" />
            <p className="text-xs font-bold text-[#8FE06A]">{items.length} TX</p>
          </div>
          <div className="space-y-2">
            {items.map((item) => {
              const positive = item.action === 'ADD';
              return (
                <div key={item.id} className="neo-card px-3 py-2">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      {item.note && (
                        <p className="mt-1 text-xs font-medium text-brutal-black/70">{item.note}</p>
                      )}
                      <p className="mt-1 text-xs font-bold text-brutal-black/60">Balance {item.balance_after} {item.ticker}</p>
                    </div>
                    <p className={`text-sm font-black ${positive ? 'text-green-600' : 'text-red-500'}`}>
                      {positive ? '+' : '-'}{item.amount_change}
                    </p>
                  </div>
                </div>
              );
